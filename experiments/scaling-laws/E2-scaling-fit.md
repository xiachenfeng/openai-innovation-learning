...
### 用户解读（2026-09-20，A4 复测，correctness 3 / mechanism 3）

1. 单调下降、不共线，loss 下降速度变慢；2. 第四点往上偏，L(N, D)，数据不充分；3. best_step 提前对应大模型样本效率更高；4. s1/s2 未收敛的影响：不清楚；5. s4 训练/val 差距大是记忆更强、过拟合。

### 教练补充

1. **不共线的两个来源要分开。** s3→s4 斜率 0.003 是真实的数据瓶颈：0.9M 训练 token 对 470 万参数不够，L(N, D) 里 D_c/D 项占主导，s3 与 s4 的 best val 几乎相等（1.529 对 1.520）。但 s1→s2 的 0.098 与 s2→s3 的 0.048 之间的弯曲有一部分是假象：s1、s2 在 12000 步上限处仍在下降，它们的 best_val 偏高，前三点的 alpha 0.074 和外推预测都不可信。若小模型训到收敛，前三点会更接近直线，第四点的偏离反而更明显。
2. **第 4 问的答案。** 所有尺寸共用 lr 3e-4 和同一步数上限，小模型吃亏，这正是 Kaplan 被 Chinchilla 批评的"固定学习率计划对不同 token 预算不公平"在 toy 尺度上的重演。Chinchilla 的做法是按每个尺寸的 token 预算单独调 cosine 长度。可选第三轮（s1、s2 换 lr 1e-3、上限 30000）就是在做这个修正。
3. **best_step 递减的两层含义。** 在 Kaplan 的意义上，样本效率指"达到同一 loss 所需 token 更少"，s4 在 2100 步以内就到了 s3 用 5700 步才到的 1.529，这一条成立；但 s4 早停在 2100 步同时也因为它更早开始记训练集（数据固定），两件事在这里叠在一起，Kaplan 的实验里数据充足，只有前一件。
4. **训练/val 差距随 N 增大**（s1 0.10，s2 0.18，s3 0.25，s4 0.38）：容量越大、越能拟合训练集，数据固定时泛化差距越大；s1 差距小是因为它连训练集都拟合不了（欠拟合）。与 E1 第二阶段 n=100 的记忆现象同源。
5. **alpha 数值不与论文比。** 全部四点 0.050、前三点 0.074，与 Kaplan 的 0.076 接近是巧合：字符级、数据重复近百轮、小模型未收敛，三个条件都与论文不同。
6. **按 D ∝ N^0.74 估算**，要让 s4 不走平，数据至少要 (4.74M / 0.79M)^0.74 ≈ 3.8 倍于 s3 饱和时的用量，tinyshakespeare 给不了，这是本实验的天花板。
...
- 恒定学习率、无 warmup 与衰减：每个点都没吃到退火收益，best_val 系统性偏高，常数不能与论文比；若退火收益随尺寸不同，α_N 也会有偏。可选对照：每个尺寸加一个长度匹配 best_step 的余弦计划重跑（见 [[scaling-laws]] §11.19，2026-09-25 追问）。
...
[[scaling-laws]]："最小示例或实验"写入第二轮结果表与解读要点；"掌握证据"记录复测；mastery 1 → 2。
