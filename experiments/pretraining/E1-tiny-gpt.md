...
### 种子复验结果已收到（2026-09-19，用户在 4090 上跑 run_seeds.sh 并 sync）

CSV 新增 24 行而非 12 行：脚本被执行了两次。同一 seed 两次结果不同（pretrained / lr 6e-5 / seed 1：0.265 与 0.304），说明 sft.py 的 seed 没有覆盖全部随机源（待排查：CUDA 非确定性、batch 采样是否用了 seeded generator）。各配置因此有 7 个样本（1337 + 3 种子 × 2 次）。另外多数类基线随 seed 在 0.24～0.32 间变动，train/test 切分也随 seed 变，组间比较应按同 seed 配对。解读留给 09-22 复测，均值表见 `weekly-reviews/2026-09-19-week1.md`。
...
- 算力：4090 单卡，smoke < 1 分钟；2000 步预训练 + 第二阶段 16 组已跑（用户未报耗时，估计 10 分钟内）；种子复验跑了两遍共 24 组（09-19）；本地 CPU 验证约 3 分钟（教练，300 步预训练 + 3 组 200 步 SFT）；
- API：无。
