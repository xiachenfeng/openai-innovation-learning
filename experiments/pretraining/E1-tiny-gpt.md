...
1. **n=100 全军覆没**：16 组里 8 组 n=100 的测试准确率全部低于多数类基线，而训练损失降到 0.001～0.1，是纯记忆。与 GPT-1"标注少时预训练优势最大"的预期相反。原因：80 万参数、2000 步字符级预训练学到的是拼写和词形，没有"谁在说话"这种篇章级特征，100 条片段不足以从头建立它；测试集 278 条的噪声也接近组间差。
